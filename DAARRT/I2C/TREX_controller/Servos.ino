void Servos()
{
  for(byte i=0;i<6;i++)                                                        // up to 6 servos are supported by T'REX controller
  {
    ...
    //
    if(servopos[i]>0) servo[i].write(servopos[i]);                 // if servopos is >0 then move servo to that position
    //if(servopos[i]<0) servo[i].write(3000+servopos[i]);            // if servopos is <0 then move servo to that position but reverse sense of direction
